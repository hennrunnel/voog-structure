...
export const LanguageTabs = ({
  ...
  return <TooltipProvider>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center">
              <TabsList className="bg-transparent h-auto p-0 border-b border-gray-200 rounded-none">
                {availableTabs.map(tab => <div key={tab} className="flex items-center">
                    <TabsTrigger value={tab} className="text-sm px-4 py-3 text-[#666] data-[state=active]:text-[#5A4FFF] data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-[#5A4FFF] data-[state=active]:shadow-none rounded-none border-b-2 border-transparent flex items-center capitalize">
              ...
              <button onClick={handleAddLanguageClick} className="text-sm px-4 py-3 text-[#666] hover:text-[#5A4FFF] rounded transition-colors">
                + Add language
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={onLanguageSettings} className="text-[#666] hover:text-[#5A4FFF] rounded p-1 transition-colors" aria-label="Language settings">
                  <PreferencesIcon />
                </button>
              </TooltipTrigger>
              <TooltipContent>
            ...
            <Button onClick={onAddPageClick} className="text-white font-semibold hover:bg-[#4A3FFF]" style={{
            padding: '8px 16px',
        ...
        {availableTabs.map(tab => <TabsContent key={tab} value={tab} className="mt-0">
            <PageTable pages={pages} onToggleExpansion={onToggleExpansion} onToggleVisibility={onToggleVisibility} onDeletePage={onDeletePage} onDuplicatePage={onDuplicatePage} onAddNestedPage={onAddNestedPage} onPageSettings={onPageSettings} onEditPage={onEditPage} onTranslatePage={onTranslatePage} />
          </TabsContent>)}
      </Tabs>

      <AddLanguageSidebar isOpen={addLanguageSidebarOpen} onClose={() => setAddLanguageSidebarOpen(false)} onAddLanguage={handleAddLanguage} />
    </TooltipProvider>;
};